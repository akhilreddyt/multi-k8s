...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=akhilreddyt/multi-client:$SHA
kubectl set image deployments/server-deployment server=akhilreddyt/multi-server:$SHA
kubectl set image deployments/worker-deployment worker=akhilreddyt/multi-worker:$SHA